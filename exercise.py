...
# Write a method called read_article that randomly selects an article and increases the
# articles "views" by one each time it's randomly selected.

def read_article(articles_list):
    i = randint(0, len(articles_list)-1)
    random_article = articles_list[i]
    random_article['views'] += 1
    print(random_article)

# Write a method called display_views that iterates through the articles and displays their
# titles and view counts, like so:

def display_views(articles):
    for article in articles_list:
        print("Title: {}, Views {}".format(article['webTitle'], article['views']))


# Test

read_article(articles_list)
read_article(articles_list)
read_article(articles_list)
read_article(articles_list)
read_article(articles_list)
